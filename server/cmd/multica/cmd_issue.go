...
func init() {
	...
	issueCreateCmd.Flags().Bool("inherit-parent-workdir", true, "Reuse parent issue's workdir/branch (default true; use --inherit-parent-workdir=false to opt out)")
...
func runIssueCreate(cmd *cobra.Command, _ []string) error {
	...
	if cmd.Flags().Changed("inherit-parent-workdir") {
		v, _ := cmd.Flags().GetBool("inherit-parent-workdir")
		body["inherit_parent_workdir"] = v
	}
	if v, _ := cmd.Flags().GetString("project"); v != "" {
		body["project_id"] = v
	}
