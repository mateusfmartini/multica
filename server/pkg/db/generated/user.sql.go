...
VALUES ($1, $2, $3)
RETURNING id, name, email, avatar_url, created_at, updated_at, onboarded_at, onboarding_questionnaire, cloud_waitlist_email, cloud_waitlist_reason, starter_content_state, custom_env
`
...
SELECT id, name, email, avatar_url, created_at, updated_at, onboarded_at, onboarding_questionnaire, cloud_waitlist_email, cloud_waitlist_reason, starter_content_state, custom_env FROM "user"
WHERE id = $1
`
...
const setUserCustomEnv = `-- name: SetUserCustomEnv :one
UPDATE "user" SET
    custom_env = $2,
    updated_at = now()
WHERE id = $1
...
type SetUserCustomEnvParams struct {
	ID        pgtype.UUID `json:"id"`
	CustomEnv []byte      `json:"custom_env"`
}

func (q *Queries) SetUserCustomEnv(ctx context.Context, arg SetUserCustomEnvParams) (User, error) {
	row := q.db.QueryRow(ctx, setUserCustomEnv, arg.ID, arg.CustomEnv)
	var i User
	err := row.Scan(
		&i.ID,
		&i.Name,
